...
async function transformComponent(node: any) {
    const transformNode = async (node: any, parent?: any) => {
        ...
        if (node.children) {
            for (let i = 0; i < node.children.length; i++) {
                const childResult = await transformNode(node.children[i], renderNode);
    ...
    const transformResult = await transformNode(node);

    return transformResult;
}

export async function getComponent(aDir: string, relativeADir: string, fileKey: string, token: string) {
    ...
    const canvases = file.document.children.filter((child: Node) => Figma.isNodeType(child, 'CANVAS')) as CANVAS[];
    const components = canvases[0].children.filter((child: Node) => child.type === 'COMPONENT');

    const componentSets = canvases[0].children.filter((child: Node) => child.type === 'COMPONENT_SET');
